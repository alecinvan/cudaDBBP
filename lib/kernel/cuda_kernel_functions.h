...
__global__ void momentumInit(float* A, int N)
{
...
__global__ void momentumUpdate(float* A, float* B, int N)
{
         int tx = blockIdx.x * blockDim.x + threadIdx.x ;
         if (tx < N)
                     A[tx] = B[tx] ;
...
__global__ void layerWeightUpdate(float* E, float* A, float* B, float* C, float* D, int wA, int wB, float u, float beta)
{
         int tx = blockIdx.x * blockDim.x + threadIdx.x ;
         int ty = blockIdx.y * blockDim.y + threadIdx.y ;
         ...
         for (int i = 0 ; i < wA ; ++i)
         {
                   float elementA = A[ty * wA + i] ;
                   float elementB = B[i * wB + tx] ;
                   value1 += elementA * elementB ;
         }

         float value2 = 0 ;
         for (int i = 0 ; i < wA ; ++i)
         {
                   float elementC = C[ty * wA + i] ;
                   float elementD = D[i * wB + tx] ;
                   value2 += elementC * elementD ;
         }
         E[ty * wB + tx] = E[ty * wB + tx] + u*value1 + u*beta*value2 ;
}
