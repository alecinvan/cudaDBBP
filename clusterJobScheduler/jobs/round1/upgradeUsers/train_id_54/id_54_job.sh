...
TRAINEDWEIGHTSTR="/TrainedWeights_360_epochs.dat"
TRAINEDWEIGHTLASTEPOCH="/TrainedWeights_360_epochs.dat"
PLEN=155659
NLEN=311318
PLENTEST=84328
NLENTEST=311227
...
{
       FHIDDEN=$(($RANDOM%51 + 80))       #newrand=$[ ( $RANDOM % ( $[ $highest - $lowest ] + 1 ) ) + $lowest ]
       SHIDDEN=$(($RANDOM%41 + 50))
...
#######            echo $SLURMDEVICEID
                ...
                echo -e "\n$CUDADBBPDIR/bin/upgradeUser/upgradeUser_54 $id $U $ALFA $INPUT $FHIDDEN $SHIDDEN $OUTPUT $EPOCH $TRAINDATA $TESTDATA $OUTCOMEDIR `hostname` $id $SLURM_JOB_ID $STAGE $TRAINEDWEIGHTSTR $TRAINEDWEIGHTLASTEPOCH $PLEN $NLEN $PLENTEST $NLENTEST $LASTJOBID $LASTOUTCOMEID $LASTWEIGHTPATH\n"
                $CUDADBBPDIR/bin/upgradeUser/upgradeUser_54 $SLURMDEVICEID $U $ALFA $INPUT $FHIDDEN $SHIDDEN $OUTPUT $EPOCH $TRAINDATA $TESTDATA $OUTCOMEDIR `hostname` $id $SLURM_JOB_ID $STAGE $TRAINEDWEIGHTSTR $TRAINEDWEIGHTLASTEPOCH $PLEN $NLEN $PLENTEST $NLENTEST $LASTJOBID $LASTOUTCOMEID $LASTWEIGHTPATH 2>&1 | tee $OUTCOMEDIR/`hostname`/$id/$SLURM_JOB_ID/output
#               sleep 3
      done
