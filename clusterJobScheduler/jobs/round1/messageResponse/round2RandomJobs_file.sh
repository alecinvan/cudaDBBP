...
#TRAINDATA="/cluster/clusterPipeline/MonteCarlo/CUDADBBP_memory_opt/data/testcode.dat"
#TESTDATA="/cluster/clusterPipeline/MonteCarlo/CUDADBBP_memory_opt/data/testcode.dat"
...
function submit_job_node()
{
       ...
       do
         ...
         #      print_arguments
               ...
               echo -e "\n$CUDADBBPDIR/bin/cuda_nn_mry_opt_no_omp_file $id $U $ALFA $INPUT $FHIDDEN $SHIDDEN $OUTPUT $EPOCH $TRAINDATA $TESTDATA $OUTCOMEDIR `hostname` $id $SLURM_JOB_ID $STAGE $TRAINEDWEIGHTSTR $TRAINEDWEIGHTLASTEPOCH $PLEN $NLEN $PLENTEST $NLENTEST $LASTJOBID $LASTOUTCOMEID $LASTWEIGHTPATH\n"
               $CUDADBBPDIR/bin/cuda_nn_mry_opt_no_omp_file $SLURMDEVICEID $U $ALFA $INPUT $FHIDDEN $SHIDDEN $OUTPUT $EPOCH $TRAINDATA $TESTDATA $OUTCOMEDIR `hostname` $id $SLURM_JOB_ID $STAGE $TRAINEDWEIGHTSTR $TRAINEDWEIGHTLASTEPOCH $PLEN $NLEN $PLENTEST $NLENTEST $LASTJOBID $LASTOUTCOMEID $LASTWEIGHTPATH 2>&1 | tee $OUTCOMEDIR/`hostname`/$id/$SLURM_JOB_ID/output
               sleep 3
      done
