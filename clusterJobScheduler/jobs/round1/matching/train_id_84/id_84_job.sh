...
TRAINDATA="/cluster/projects/cuda_neural_net/train_data_set/data/final/trainingdate/matching/matching_10_24_2013/matching_10_24_2013_train_small.dat"
TESTDATA="/cluster/projects/cuda_neural_net/train_data_set/data/final/trainingdate/matching/matching_10_24_2013/matching_10_24_2013_test_small.dat"
TRAINEDWEIGHTSTR="/TrainedWeights_60_epochs.dat"
TRAINEDWEIGHTLASTEPOCH="/TrainedWeights_60_epochs.dat"
PLEN=4354799
NLEN=4354800
PLENTEST=1088700
NLENTEST=1088700
...
function submit_job_node()
{
       ...
       do
               ...
               if [ -d $OUTCOMEDIR/`hostname`/$id/$SLURM_JOB_ID ] ; then
                ...
                echo -e "\n$CUDADBBPDIR/bin/matching/matching_id_84 $id $U $ALFA $INPUT $FHIDDEN $SHIDDEN $OUTPUT $EPOCH $TRAINDATA $TESTDATA $OUTCOMEDIR `hostname` $id $SLURM_JOB_ID $STAGE $TRAINEDWEIGHTSTR $TRAINEDWEIGHTLASTEPOCH $PLEN $NLEN $PLENTEST $NLENTEST $LASTJOBID $LASTOUTCOMEID $LASTWEIGHTPATH $BETA\n"
                $CUDADBBPDIR/bin/matching/matching_id_84 $SLURMDEVICEID $U $ALFA $INPUT $FHIDDEN $SHIDDEN $OUTPUT $EPOCH $TRAINDATA $TESTDATA $OUTCOMEDIR `hostname` $id $SLURM_JOB_ID $STAGE $TRAINEDWEIGHTSTR $TRAINEDWEIGHTLASTEPOCH $PLEN $NLEN $PLENTEST $NLENTEST $LASTJOBID $LASTOUTCOMEID $LASTWEIGHTPATH $BETA 2>&1 | tee $OUTCOMEDIR/`hostname`/$id/$SLURM_JOB_ID/output
#               sleep 3
      done
