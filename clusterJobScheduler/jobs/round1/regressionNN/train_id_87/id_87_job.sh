...
TRAINDATA="/cluster/projects/cuda_neural_net/train_data_set/data/csv/regressionNN/usersTimeOnSite_scale_train_dataset.dat"
TESTDATA="/cluster/projects/cuda_neural_net/train_data_set/data/csv/regressionNN/usersTimeOnSite_scale_test_dataset.dat"
...
{
       FHIDDEN=$(($RANDOM%10 + 40))       #newrand=$[ ( $RANDOM % ( $[ $highest - $lowest ] + 1 ) ) + $lowest ]
       SHIDDEN=$(($RANDOM%15 + 25))
...
function submit_job_node()
{
       ...
       do
               ...
               if [ -d $OUTCOMEDIR/`hostname`/$id/$SLURM_JOB_ID ] ; then
                ...
                echo -e "\n$CUDADBBPDIR/bin/regressionNN/regression_id_87 $id $U $ALFA $INPUT $FHIDDEN $SHIDDEN $OUTPUT $EPOCH $TRAINDATA $TESTDATA $OUTCOMEDIR `hostname` $id $SLURM_JOB_ID $STAGE $TRAINEDWEIGHTSTR $TRAINEDWEIGHTLASTEPOCH $BETA $NLEN $MEAN_TEST $LASTJOBID $LASTOUTCOMEID $LASTWEIGHTPATH \n"
                $CUDADBBPDIR/bin/regressionNN/regression_id_87 $SLURMDEVICEID $U $ALFA $INPUT $FHIDDEN $SHIDDEN $OUTPUT $EPOCH $TRAINDATA $TESTDATA $OUTCOMEDIR `hostname` $id $SLURM_JOB_ID $STAGE $TRAINEDWEIGHTSTR $TRAINEDWEIGHTLASTEPOCH $BETA $MEAN $MEAN_TEST $LASTJOBID $LASTOUTCOMEID $LASTWEIGHTPATH  2>&1 | tee $OUTCOMEDIR/`hostname`/$id/$SLURM_JOB_ID/output
#               sleep 3
      done
