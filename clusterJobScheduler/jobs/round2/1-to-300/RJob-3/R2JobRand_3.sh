...
function submit_job_node()
{
       ...
       do
         ...
         #      print_arguments
               ...
               echo -e "\n$CUDADBBPDIR/bin/cuda_bp_cluster_round2 $id $U $ALFA $INPUT $FHIDDEN $SHIDDEN $OUTPUT $EPOCH $TRAINDATA $TESTDATA $OUTCOMEDIR `hostname` $id $SLURM_JOB_ID $STAGE $TRAINEDWEIGHTSTR $TRAINEDWEIGHTLASTEPOCH $PLEN $NLEN $LASTJOBID $LASTWEIGHTPATH $LASTOUTCOMEID\n"
               $CUDADBBPDIR/bin/cuda_bp_cluster_round2 $SLURMDEVICEID $U $ALFA $INPUT $FHIDDEN $SHIDDEN $OUTPUT $EPOCH $TRAINDATA $TESTDATA $OUTCOMEDIR `hostname` $id $SLURM_JOB_ID $STAGE $TRAINEDWEIGHTSTR $TRAINEDWEIGHTLASTEPOCH $PLEN $NLEN $LASTJOBID $LASTWEIGHTPATH $LASTOUTCOMEID 2>&1 | tee $OUTCOMEDIR/`hostname`/$id/$SLURM_JOB_ID/output
#               sleep 3
      done
